...
namespace cbk::ecs {
	...
	inline void to_json(nlohmann::json& j, const CCamera& c) {
		j = {
			{ "type", c.Type }, { "is_active", c.IsActive },   { "fov_y", c.FovY }, { "near", c.Near },
			{ "far", c.Far },   { "ortho_size", c.OrthoSize },
	...
	inline void from_json(const nlohmann::json& j, CModel& c) {
		...
		switch (c.Kind) {
			case common::MaterialKind::Phong:
				material = rendering::PhongMaterial::create();
				break;
			case common::MaterialKind::PBR:
				material = rendering::PBRMaterial::create();
				break;
			default:
