...
namespace cbk {
	...
	class LayerStack {
	  public:
		...
		[[nodiscard]] std::vector<Scope<Layer>>::iterator end() {
			return m_Layers.end();
		}

	  private:
		std::vector<Scope<Layer>> m_Layers; // Actual structure for the layers
		unsigned int m_LayerInsertIndex;    // Custom iterator for the layers. The idea is to always push a new layer on top of other layers
		                                    // but under every overlay. For the overlays, push them always on top of everything
	};
} // namespace cbk
