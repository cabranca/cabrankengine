...
#ifdef CBK_RENDERER_OPENGL
		return createRef<platform::opengl::OpenGLTexture2D>(path, sRGB);
#elif defined(CBK_RENDERER_METAL)
		return createRef<platform::metal::MetalTexture2D>(path, sRGB);
#elif defined(CBK_RENDERER_VULKAN)
		return createRef<platform::vk::VulkanTexture>(path, sRGB);
#else
		CBK_CORE_ASSERT(false, "No renderer API defined!");
		return nullptr;
#endif
