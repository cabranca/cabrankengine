...
namespace cbk::platform::vk {
	...
	VulkanUniformBuffer::VulkanUniformBuffer(uint32_t size, uint32_t binding) : m_Size(size) {
		...
		if (vkResult != VK_SUCCESS) {
			CBK_CORE_ERROR("VulkanUniformBuffer: vkCreateDescriptorSetLayout failed ({})", static_cast<int>(vkResult));
			return;
		}
		...
		if (vkResult != VK_SUCCESS) {
			CBK_CORE_ERROR("VulkanUniformBuffer: vkCreateDescriptorPool failed ({})", static_cast<int>(vkResult));
			return;
		}
		...
		if (vkResult != VK_SUCCESS) {
			CBK_CORE_ERROR("VulkanUniformBuffer: vkAllocateDescriptorSets failed ({})", static_cast<int>(vkResult));
			return;
		}
