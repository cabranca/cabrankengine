...
namespace cbk::platform::vk {
	...
	void VulkanTextMaterial::initSharedResourcesIfNeeded() {
		...
		// arrive in world space so the model-matrix push constant goes unread — but
		// VulkanRendererAPI pushes it unconditionally, so the layout must still
		// declare the range or vkCmdPushConstants is invalid.
		...
		std::array<VkVertexInputAttributeDescription, 4> vertexAttributes = {
			VkVertexInputAttributeDescription{ .location = 0, .binding = 0, .format = VK_FORMAT_R32G32B32_SFLOAT,    .offset = offsetof(TextVertex, position) },
			VkVertexInputAttributeDescription{ .location = 1, .binding = 0, .format = VK_FORMAT_R32G32B32A32_SFLOAT, .offset = offsetof(TextVertex, color)    },
			VkVertexInputAttributeDescription{ .location = 2, .binding = 0, .format = VK_FORMAT_R32G32_SFLOAT,       .offset = offsetof(TextVertex, texCoord) },
			VkVertexInputAttributeDescription{ .location = 3, .binding = 0, .format = VK_FORMAT_R32_SFLOAT,          .offset = offsetof(TextVertex, texIndex) },
		};
